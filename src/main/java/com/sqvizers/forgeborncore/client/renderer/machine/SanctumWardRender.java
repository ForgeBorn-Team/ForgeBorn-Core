...
public class SanctumWardRender extends DynamicRender<WorkableMultiblockMachine, SanctumWardRender> {
    ...
    public SanctumWardRender() {
        ModelEventHelper.registerBakeEventListener(true, (modelLocation, bakedModel, rootModel, modelBakery) -> {
            if (modelLocation.id().equals(SPHERE_MODEL_RL)) {
    ...
    @Override
    public DynamicRenderType<WorkableMultiblockMachine, SanctumWardRender> getType() {
        return TYPE;
    }

    @Override
    public void render(WorkableMultiblockMachine machine, float partialTick, PoseStack poseStack,
        ...
        List<BakedQuad> quads = sphereModel.getQuads(null, null, random);
